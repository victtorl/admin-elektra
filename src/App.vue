<template>
  ...
  <div>
            ...
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-indigo-600 px-6 pb-4">
                <div class="flex h-16 shrink-0 items-center">
                  <img class="h-8 w-auto" src="../src/assets/logoadmin.png" alt="Your Company" />
                </div>
    ...
    <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
      <!-- Sidebar component, swap this element with another sidebar if you like -->
      <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-elecktranegro px-6 pb-4">
        <div class="flex h-16 shrink-0 items-center">
          <img class="h-8 w-auto" src="../src/assets/logoadmin.png" alt="Your Company" />
          <p class="text-white pl-4 font-bold" >Administrador</p>
        </div>
        <nav class="flex flex-1 flex-col">
          <ul role="list" class="flex flex-1 flex-col gap-y-7">
            <li>
              <ul role="list" class="-mx-2 space-y-1">
                <li v-for="item in navigation" :key="item.name">
                  <a :href="item.href" :class="[item.href==route.fullPath ? 'bg-elecktraamarillo text-white' : 'text-indigo-200 hover:bg-elecktraamarillo hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                    <component :is="item.icon" :class="[item.current ? 'text-white' : 'text-indigo-200 group-hover:text-white', 'size-6 shrink-0']" aria-hidden="true" />
                    {{ item.name }}
                  </a>
                </li>
    ...
    <div class="lg:pl-72">
      <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-elektraplomo px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
...
const navigation = [
  ...
  { name: 'Administrar contenido', href: '/administrar-contenido', icon: PlusCircleIcon, current: false },
